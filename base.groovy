pipeline {
  ...
  parameters {
    string(
      ...
      defaultValue:'758526784474.dkr.ecr.us-east-1.amazonaws.com/base-playwright-py-ftd-qa:develop',
    ...
    string(
      name: 'command_run',
      defaultValue:'pytest',
      description: 'Where you set command'
    )
    string(
      name: 'command_browser',
      defaultValue:'--browser chromium',
      description: 'Where you set browser'
    )
    string(
      name: 'env',
      defaultValue:'dev',
          ...
          catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
            ...
            configFileProvider([configFile(fileId: "${params.file_var}", variable:"${params.file_var}")]) {
              wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'xterm']) {
                sh "cd /base-playwright-py-ftd-qa && . \$${params.file_var} >> /dev/null && ${params.command_run} ${params.command_browser}"
              }
            }
          }
        }
    ...
    post {
      always{
      echo "TESTS FINISHED"
        sh 'cd /base-playwright-py-ftd-qa/reports && zip allure-results allure-results/*'
        sh 'cp /base-playwright-py-ftd-qa/reports/allure-results.zip ${WORKSPACE}/'
        sh 'cd /base-playwright-py-ftd-qa/reports && rm -R allure-results.zip'
...
//              sh 'cd /base-playwright-py-ftd-qa && python3 slack/slack_finish_testes_notify.py'
